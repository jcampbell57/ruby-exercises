...
def count_empty_seats(chart)
  ...
  chart.flatten.count - chart.flatten.compact.count
end
...
def find_favorite(array_of_hash_objects)
  ...
  array_of_hash_objects.detect(ifnone = nil) do |hash|
    if hash[:is_my_favorite?] == true
      return hash
    end
  end

  # array_of_hash_objects.select { |hash, data| data[:is_my_favorite?] == true }
  # array_of_hash_objects.collect { |hash| hash if hash[:is_my_favorite?] == true }
